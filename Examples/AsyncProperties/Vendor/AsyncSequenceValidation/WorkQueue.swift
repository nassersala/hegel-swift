...
/// Added by hegel-swift: the order in which a batch of ready items runs
/// is a policy instead of a fixed sort. Every Swift job the operators
/// enqueue (captured through the global hook), every input emission and
/// every clock wakeup is an item, so the policy is the schedule.
final class SchedulePolicy: @unchecked Sendable {
  /// Given the ready batch (in the default order) and the number of prior
  /// choice points, the index to run next. nil = default order.
  var pick: ((_ ready: [String], _ choice: Int) -> Int)?
  var choicePoints = 0
  var maxWidth = 0
  var log: [String] = []
  init(pick: ((_ ready: [String], _ choice: Int) -> Int)? = nil) { self.pick = pick }
}
...
struct WorkQueue: Sendable {
  ...
  init(policy: SchedulePolicy = SchedulePolicy()) {
    self.policy = policy
  }

  /// Runs a batch. Items that become ready while running (jobs resumed
  /// by an item) join the ready set at once, after the items already
  /// waiting, so the default order is the upstream order; a policy may
  /// run any ready item, including holding one back across several
  /// steps. A ready set of two or more is a choice point.
  func runBatch(_ sorted: [Item]) {
    var items = sorted
    while !items.isEmpty {
      defer {
        var more: [Item] = state.withCriticalRegion { $0.drain() }
        more.sort()
        items += more
      }
      var index = 0
      if items.count > 1 {
        policy.maxWidth = max(policy.maxWidth, items.count)
        let ready = items.map(\.description)
        if let pick = policy.pick {
          index = pick(ready, policy.choicePoints)
          precondition(items.indices.contains(index), "policy chose \(index) of \(items.count)")
        }
        if index != 0 {
          policy.log.append("choice \(policy.choicePoints) @\(now): ran \(ready[index]) before \(ready[0])")
        }
        policy.choicePoints += 1
      }
      items.remove(at: index).run()
    }
  }
