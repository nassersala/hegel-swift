...
3. **Differential conformance** (`Conformance/` + `Scripts/conformance.sh`) — seven draw programs, each written once per binding, run under the same seed in hegel-swift, hegel-go, hegel-rust and dialectic; the transcripts must match byte for byte. Swift, Go and dialectic load the *same* vendored libhegel binary (hegel-go accepts it via `HEGEL_LIBHEGEL_PATH`; both pin 0.32.5), and the Rust column links its own copy of that engine version, so it is the reference the engine's own language gives. The programs cover the primitive draws, the big-integer ABI, unrestricted and codec text, regex and email, collections, and a state machine with and without rejected steps. The choice-sequence model makes cross-language determinism directly checkable, and the check found two divergences: `array(of:)` here did not open the `HEGEL_LABEL_LIST` span the other bindings open, and from case 11 of the lists program the drawn lists differed (the engine's generation reads span structure; fixed); and hegel-go never calls `hegel_state_machine_rule_rejected`, so a rejected rule counts against its step budget where every other binding redraws ([hegeldev/hegel-go#135](https://github.com/hegeldev/hegel-go/issues/135)). Everything else matched on the first run.
...
- [x] Differential conformance vs hegel-go, hegel-rust and dialectic: seven draw programs, identical seed → identical draw transcript (`Scripts/conformance.sh`); found the missing list span here and the unreported rule rejection in hegel-go
