...
**Zero-downtime deployment, from a talk.** Scott Wlaschin's "TLA+ for programmers" ends with Hillel Wayne's deployment: servers go from v1 to v2, each offline while it upgrades, at least one online at every moment, and clients must never see two versions. The talk fixes it twice by hand and runs TLC each time. Drawn first on two servers, the row after the first upgrade finishes has two servers up on two versions and nothing in the row says which one clients see, so the balancer is the variable that had to be written in, and "online" is not a phase of a server but the set at the balancer's version. That makes the start guard read the online set, `|Online \ {s}| ≥ k`, not "is anyone else offline". `Deploy.swift` is the relation with `k` as a parameter; the talk's two designs are kept and refuted in two steps each on the smallest fleet, against the invariant the talk had at that round:

```
anyServer, n = 2, k = 1, refuted after start 0, start 1: [s0: off, s1: off]  online []
oneOffline, n = 2, k = 1, refuted after start 0, finish 0: [s0: v2, s1: v1]  online [s0, s1]
```

Two things the relation says that the talk did not. The guard is a batch: with five servers and `k = 2`, drawn behaviours take three offline at once and never four, and `Rollout.batched` reads that number off the guard where the talk's procedure takes one at a time. And the relation has a precondition, `n ≥ 2k`: at the switch the online set becomes the v2 servers, so `k` of them must be up while the v1 side still keeps `k`, and three servers cannot keep two online. The drawn behaviour stops at `[s0: v2, s1: v1, s2: v1 | lb: v1]` with only idle enabled; TLC reports the same state as a deadlock in seven states. Two rollouts refine, and the seeded bugs are the first step that is not a step: switching on the first v2 server needs `k = 2` and four servers to bite, one more in the batch than the guard allows needs two. The talk's first fix as code, a balancer never switched, records only steps and stops short, and the relation names the one step the code refuses. Under the controlled scheduler the guard is the talk's producer-consumer bug: a check and a commit with an `await` between them passes under the default schedule and is found at one deviation on two servers, the second start committed with itself the only server online. `TLA/Deploy.tla` is the twin: `N = 5, K = 2` gives 262 states with both invariants, the batch bound, and `<>[]Done` under weak fairness. The stutter is where the talk's counter lesson lands: `idle` is a step of the relation at every state, so "eventually done" is not a trace property, and what the trace says instead is that every completed behaviour has exactly `2n + 1` steps that are not idle.

**A stream function, and what the modal types would have said.** Lively RaTT makes three things type errors in reactive code: reading the future, since a later value cannot be advanced before the tick; holding the past, since only stable values survive one; and an unproductive tick, since every recursive call sits behind one. Swift has none of that, so the question is what a tester recovers. `Causal.swift` is the relation, drawn first as a moving average over three ticks: a read carries the tick it reads, an emit carries the tick it is for and the inputs the function still holds, and a tick is a step only if the current tick has its output. The clock is the environment on the scheduler's fake clock, which fires only when nothing is ready, so "the tick came and the output had not" is a state the run reaches. Six stream functions on a tape under drawn schedules, with the source running one ahead or not:

```
runningAverage advancing on the data, x = [0, 0], prefetch true: step 2: read 1 @0 is not a Next step: reads tick 1 at tick 0: the future
keepsEverything, x = [0, 0]: step 4: emit 1 holding [0, 1] @1 is not a Next step: holds tick 0 at tick 1, older than the window of 0
stalls: tick @1 is not a Next step: tick 2 with no output for tick 1: unproductive
spins: runaway at every budget in 24...64 steps, no moment refused
```

The first line is the run's own finding, kept as a refutation: as first written, every function advanced when its next input arrived, and the running average, which the types accept, read the future under a source one ahead, on the default schedule, at two ticks. The function must advance on the tick, not on the data; that is what advancing a later value under a tick means, and with that wait both accepted functions refine under 200 schedules with every output the reference. The lookahead is refused two ways depending on the environment, as the future when the source is ahead and as an unproductive tick when it is not, the same bug with two reports. The last line is the gap. A function that polls instead of waiting never idles, so the clock never fires and no tick is ever refused; every recorded moment is a step, and all the run can say is that a step budget ran out. No budget separates not yet from never, and the type checker would have said never. So of the three guarantees, the two safety ones come back as the story the type error would have told, and productivity comes back as a bound.
...
`Examples/SchemaMigrations` is a category with real SQLite as the meaning. Objects are schemas, arrows are migrations (add, drop, rename), composition concatenates and squashes, and two migrations are equal when SQLite reports the same schema after them. `Laws.category` checks the squash under that equality; `Laws.functor` checks that running the squashed script does to SQLite what running the two scripts in turn does, which also pins the in-memory schema model to SQLite's behaviour (added columns go to the end, and order is observable). A planted `drop x … add x` rule shrinks to a two-step script. The example's own rename-chain rule was unsound in its first draft, found by the category laws: `rename b→c; drop a; rename c→a` merged to a rename onto a name still in use.
...
3. **Differential conformance** (`Conformance/` + `Scripts/conformance.sh`) — seven draw programs, each written once per binding, run under the same seed in hegel-swift, hegel-go and hegel-rust; the transcripts must match byte for byte. Swift and Go load the *same* vendored libhegel binary (hegel-go v0.9.5 accepts it via `HEGEL_LIBHEGEL_PATH`; both pin 0.43.1), and the Rust column (hegeltest 0.46.1) links its own copy of that engine version, so it is the reference the engine's own language gives. The programs cover the primitive draws, the big-integer ABI, unrestricted and codec text, regex and email, collections, and a state machine with and without rejected steps. hegel-swift matches the reference on all seven. The choice-sequence model makes cross-language determinism directly checkable, and the check keeps finding things. On libhegel 0.32: `array(of:)` here did not open the list span the other bindings open, and from case 11 of the lists program the drawn lists differed (the engine's mutation reads span structure; fixed); and hegel-go never called `hegel_state_machine_rule_rejected` ([hegeldev/hegel-go#135](https://github.com/hegeldev/hegel-go/issues/135), fixed upstream since). On the move to 0.43, where span labels became hashes of generator names: every list here shared one label while the reference folds the element generator's label in, so a list of integers and a list of booleans were interchangeable to the mutator, and the lists program parted at case 11 again (fixed: `drawCollection` folds the element type in). hegel-go v0.9.5 now differs from the reference on three programs, listed with their causes under `KNOWN` in the script: its list labels ignore the element generator, which its maintainers found independently ([hegeldev/hegel-go#156](https://github.com/hegeldev/hegel-go/issues/156)), and it runs a sequential state machine through its concurrent path, drawing rules on a clone of the test case where the reference draws on the root handle. The dialectic column is optional and off: dialectic v0.1 is written against the 0.32 ABI.
...
- [x] Compile against real `libhegel` (0.43.1, built from the pinned hegel-rust tag `libhegel-v0.43.1`); suite green incl. shrink-to-known-minimum
...
- [x] Differential conformance vs hegel-go and hegel-rust (dialectic until it is ported to the 0.43 ABI): seven draw programs, identical seed → identical draw transcript (`Scripts/conformance.sh`); found the missing list span and, on 0.43, the element-blind list label here, and the unreported rule rejection in hegel-go
- [x] libhegel 0.43.1: the round-based state-machine protocol (sequential, every invariant always-check), name-hashed span labels (`TestCase.label(_:)`, `label(combining:)`), nanosecond `TimeOfDay`; `Settings.mode == .singleTestCase` is now a test-case budget of one, as upstream replaced the mode it set
...
## License
MIT — see [LICENSE](LICENSE). `Vendor/CHegel.xcframework` contains libhegel binaries built unmodified from [hegeldev/hegel-rust](https://github.com/hegeldev/hegel-rust) `libhegel-v0.43.1` (MIT, Antithesis, LLC); the upstream notice is included in LICENSE. This binding is unofficial and not affiliated with Antithesis.
